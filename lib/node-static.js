...
function Server(address, serverOptions) {
	...
	/* public functions */
	this.serveFile = function(filePath, statusCode, headers, request, response) {
		...
		_.extend(headers, options.headers, serverOptions.headers);


		var extName = options.extension = path.extname(filePath), 
			pathNameRaw = 	options.rawPath = filePath,
			pathNameAbs = options.absPath = path.join(directory, filePath),
			fileStat,
			callback = function(){console.log('my callback');};
...
function processRequestHeaders(options, done, request, response){
	...
	async.series([
			...
			function(done1) {
				if(request.headers['accept-encoding'] && /\bgzip\b/i.test(request.headers['accept-encoding'])) {
					var regExpCondition = _.isRegExp(options.mustGzip) && options.mustGzip.test(mime.lookup(options.extension));
				
					if(!(regExpCondition || options.mustGzip === true )) options.mustGzip = false;
					//the true case, is ok
				} else {
						options.mustGzip = false;
				}
				done1();
			},
...
function generateResponseHedears(options, done, request, response){
		async.series([
				...
				function(done1) {
					...
					options.mustGzip && ( options.headers['content-encoding'] = 'gzip');
					done1();
				}
		],
		function(err) {
...
function generateResponseBody(options, request, response, callback){
	switch(request.method) {
		...
		case "GET":
				...
				var tempStream = readStream = fs.createReadStream(options.absPath);

				if(options.mustGzip) {
					tempStream = readStream.pipe(zlib.createGzip());
				}
				tempStream.pipe(response);
